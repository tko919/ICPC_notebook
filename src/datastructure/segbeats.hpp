struct SegtreeBeats {
    ...
    struct alignas(32) Node {
        ll sum = 0;
        ll a1[2] = {}, a2[2] = {-INF, INF}, ac[2] = {1, 1}, add = 0;
    };
    vector<Node> v;
    ll n, log, e[3] = {-INF, INF, 0};
    SegtreeBeats() {}
  ...
  private:
    ...
    void push(int cmd, int k, ll x) {
        ...
        if (p.a1[cmd ^ 1] == p.a1[cmd])
            p.a1[cmd ^ 1] = x;
        if (p.a2[cmd ^ 1] == p.a1[cmd])
            p.a2[cmd ^ 1] = x;
        p.a1[cmd] = x;
    }
    void push(int k) {
        Node &p = v[k];
        if (p.add) {
            rep(t, 0, 2) push_add(k * 2 + t, p.add);
            p.add = 0;
        }
        rep(t, 0, 2) rep(s, 0, 2) if (cmp(t, v[k * 2 + s].a1[t], p.a1[t]))
            push(t, k * 2 + s, p.a1[t]);
    }
    void subtree_ch(int cmd, int k, ll x) {
        if (!cmp(cmd, v[k].a1[cmd], x))
            return;
        if (cmp(cmd, x, v[k].a2[cmd])) {
            return push(cmd, k, x);
        }
        push(k);
        rep(t, 0, 2) subtree_ch(cmd, k * 2 + t, x);
        update(k);
    }
    template <int cmd> inline void _apply(int k, ll x) {
        rep(i, 0, 2) if (cmd >> i & 1) subtree_ch(i, k, x);
        if constexpr (cmd == 0)
            push_add(k, x);
    }
