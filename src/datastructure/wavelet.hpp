template <typename T> struct WaveletMatrix {
    struct BitVector {
        ...
        int rank(int k, bool f = 1) {
            int ret = rui[k >> 6] + __builtin_popcountll(
                                        buf[k >> 6] & ((1ull << (k & 63)) - 1));
            if (!f)
                return k - ret;
            else
    ...
    int rank(int L, int R, T x) { // x in [L,R)
        if ((T(1) << lg) <= x)
            return 0;
        for (int d = lg - 1; d >= 0; d--) {
            bool f = (x >> d & 1);
    ...
    T quantile(int L, int R, int k) { // kth elem in [L,R)
        T ret = 0;
        for (int d = lg - 1; d >= 0; d--) {
    ...
    int freq(int L, int R, T x) { // <x in [L,R)
        if ((T(1) << lg) <= x)
            return R - L;
        int ret = 0;
        for (int d = lg - 1; d >= 0; d--) {
            bool f = (x >> d & 1);
            if (f)
                ret += buf[d].rank(R, 0) - buf[d].rank(L, 0);
            L = buf[d].rank(L, f) + (f ? mid[d] : 0);
            R = buf[d].rank(R, f) + (f ? mid[d] : 0);
        }
        return ret;
    ...
    // [a,b) in [L,R)
    int freq(int L, int R, T a, T b) { return freq(L, R, b) - freq(L, R, a); }
    // largest <=x in [L,R)
