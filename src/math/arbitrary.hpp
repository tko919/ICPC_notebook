...
template <typename T>
vector<T> ArbitraryMult(const vector<T> &a, const vector<T> &b, bool same = 0) {
    ...
    vector<M1> a1(ALL(aa)), b1(ALL(bb)), c1 = mult<M1, 3>(a1, b1, same);
    vector<M2> a2(ALL(aa)), b2(ALL(bb)), c2 = mult<M2, 6>(a2, b2, same);
    vector<M3> a3(ALL(aa)), b3(ALL(bb)), c3 = mult<M3, 7>(a3, b3, same);
    for (M1 x : c1)
