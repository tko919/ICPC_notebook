class GeneralWeightedMatching {
    ...
    void update(int u, int v) {
        if (!slack[v] or dist(G[u][v]) < dist(G[slack[v]][v]))
            slack[v] = u;
    }
    void recalc(int v) {
        slack[v] = 0;
  ...
  public:
    GeneralWeightedMatching(int _n)
        : n(_n), m(n), in(0), G(n * 2, vector<E>(n * 2)), mate(n * 2),
          root(n * 2), used(n * 2), flower(n * 2),
          belong(n * 2, vector<int>(n * 2)), dual(n * 2) {
        rep(i, 0, n + 1) {
            root[i] = i;
            belong[i][i] = i;
            if (i)
